...
class Formula:
	...
	def save_symmetric_image(self, path, class_name = "latex", include_static_style = True):
		...
		class_property = '' if class_name is None else ' class="' + class_name + '"'
		static_style_0 = 'position:relative;display:inline-block;vertical-align:0;' if include_static_style else ''
		...
		return f'<span{class_property} style="{static_style_0}width:{round(self.em_width, 4)}em;height:{round(em_height, 4)}em;margin-bottom:{round(self.em_height_bottom, 4)}em;"><img{static_style_1} src="',\
		       '"/></span>'
